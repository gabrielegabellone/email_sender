...
class EmailsTestCase(TestCase):
    ...
    def test_send_email_to_all_subscribers_ok(self):
        """Tests the correct response of the endpoint when sending emails is successful and that the email is sent to
        all subscribers."""
        response = self.client.post('/emails/send_to_all/', data={'subject': 'test_subject', 'message': 'test_message'}, format='json')
    ...
    def test_send_email_to_all_subscribers_error_sending_email(self, mock_send_mail):
    ...
    def test_send_email_to_all_subscribers_missing_subject(self):
        """Tests the correct response of the endpoint when subject is missing."""
        response = self.client.post('/emails/send_to_all/', data={'recipients': ['test@gmail.com'], 'message': 'test_message'}, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'{"msg":"Emails successfully sent"}', response.content)
    ...
    def test_send_email_to_all_subscribers_missing_message(self):
        """Tests the correct response of the endpoint when message is missing."""
        response = self.client.post('/emails/send_to_all/', data={'subject': 'test_subject'}, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'{"msg":"Emails successfully sent"}', response.content)
    ...
    def test_send_email_to_specific_subscribers_ok(self):
        """Tests the correct response of the endpoint when sending emails is successful and that the email is sent to
        the correct subscribers."""
        response = self.client.post('/emails/send/', data={'recipients': [22, 23], 'subject': 'test_subject', 'message': 'test_message'}, format='json')
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'{"msg":"Emails successfully sent"}', response.content)
    ...
    def test_send_email_to_specific_subscribers_missing_recipients(self):
        """Tests the correct response of the endpoint when recipients is missing."""
        response = self.client.post('/emails/send/', data={'subject': 'test_subject', 'message': 'test_message'}, format='json')
        self.assertEqual(response.status_code, 400)
        self.assertIn(b'{"msg":"Recipient list required"}', response.content)

    def test_send_email_to_specific_subscribers_emails_partially_sent(self):
        """Tests the correct response of the endpoint when emails are partially sent."""
        response = self.client.post('/emails/send/', data={'recipients': [12, 13], 'subject': 'test_subject', 'message': 'test_message'}, format='json')
